...
RETURNS boolean
LANGUAGE plpgsql
SECURITY INVOKER -- This function is called by RLS, so it should run as the invoking user
AS $$
DECLARE
    current_user_id uuid := auth.uid();
    current_user_role public.role_type;
BEGIN
    ...
    IF current_user_role IN ('manager', 'desk') THEN
        IF lead_row.assigned_to IS NOT NULL AND lead_row.assigned_to IN (SELECT public.get_all_subordinate_ids(current_user_id)) THEN
            RETURN TRUE;
        END IF;
    END IF;
