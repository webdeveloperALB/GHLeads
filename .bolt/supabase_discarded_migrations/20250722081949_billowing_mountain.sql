...
AS $$
BEGIN
    RETURN (SELECT raw_app_meta_data->>'role' FROM auth.users WHERE id = user_id) = 'admin';
END;
$$;
...
USING (
    ...
    OR
    (
        (SELECT role FROM public.user_profiles WHERE id = auth.uid()) IN ('manager', 'desk') AND
